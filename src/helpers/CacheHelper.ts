import { ChurchInterface } from "../mobilehelper";
  ...
  static loadFromStorage = async () => {
    ...
    data.forEach(item => {
      ...
      if (val) {
        switch (item[0]) {
          case "CHURCH_DATA":
            this.church = JSON.parse(item[1] || "");
            break;
          case "fcmToken":
            this.fcmToken = item[1] || "";
            break;
          case "RECENT_CHURCHES":
            this.recentChurches = JSON.parse(item[1] || "");
            break;
          case "USER_DATA":
            UserHelper.user = JSON.parse(item[1] || "");
            break;
        }
      }
    });
