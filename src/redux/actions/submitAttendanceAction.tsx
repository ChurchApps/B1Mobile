...
export function submitAttendanceData(serviceId: any, peopleIds: any, token: any, pendingVisits: any, callback: any) {
    return (dispatch:any) => {
        dispatch({
            type: SUBMIT_ATTENDANCE_FETCH
        });
        axios.post(API.ATTENDANCE_URL + '?serviceId=' + serviceId + '&peopleIds=' + peopleIds, pendingVisits, { headers: {"Authorization" : `Bearer ${token}`} })
            .then(res => {
                dispatch({
