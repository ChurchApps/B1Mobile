...
export default function CreateEvent(props: Props) {
  ...
  const handleSave = () => {
    if (props.event.recurrenceRule) setRecurrenceModalType("save");
      ...
      ApiHelper.post("/events", [ev], "ContentApi").then(data => {
        setLoading(false); if (props.onDone) props.onDone();
      });
    }
  }
    ...
    switch (editType) {
      case "this":
        ...
        ApiHelper.post("/eventExceptions", [exception], "ContentApi").then((data) => {
          ...
          ApiHelper.post("/events", [oneEv], "ContentApi").then(() => { if (props.onDone) props.onDone(); setShowEventEditModal(false) });
        });
        break;
      case "future":
        ...
        ApiHelper.post("/events", [originalEv, newEvent], "ContentApi").then(() => { if (props.onDone) props.onDone(); setShowEventEditModal(false) });
        break;
      case "all":
        const allEv = { ...event };
        allEv.recurrenceRule = rRule;
        ApiHelper.post("/events", [allEv], "ContentApi").then(() => { if (props.onDone) props.onDone(); setShowEventEditModal(false) });
        break;
    }
  }
  ...
  const handleRecurringDelete = (editType: string) => {
    switch (editType) {
      case "this":
        const exception: EventExceptionInterface = { eventId: event.id, exceptionDate: event.start };
        ApiHelper.post("/eventExceptions", [exception], "ContentApi").then(() => { if (props.onDone) props.onDone(); setShowEventEditModal(false) });
        break;
      case "future":
        const ev = { ...event };
        ...
        ApiHelper.post("/events", [event], "ContentApi").then(() => { if (props.onDone) props.onDone(); setShowEventEditModal(false) });
        break;
      case "all":
        ApiHelper.delete("/events/" + event.id, "ContentApi").then(() => { if (props.onDone) props.onDone(); setShowEventEditModal(false) });
        break;
    }
  }
  ...
  const handleChange = (type: string, e: Date) => {
    const env = { ...event };
    switch (type) {
      case "start": env.start = DateHelper.toDate(e); break;
      case "end": env.end = DateHelper.toDate(e); break;
    }
    setEvent(env);
  }
  ...
  return (
    <View>
      <View style={styles.eventType}>
        <CheckBox
          onPress={() => { setAllDay(!allDay), setEvent({ ...event, allDay: !allDay }); }}
          title="All Day"
          isChecked={allDay}
        />
        <CheckBox
          onPress={() => { handleToggleRecurring(!recurring); }}
          title="Recurring"
          isChecked={recurring}
        />
      </View>
      ...
      {!event?.id ? (
        ...
        >
          {loading ?
            ...
            <Text style={globalStyles.roundBlueButtonText}>{"Save"}</Text>
          }
        </TouchableOpacity>
      ) : (
      ...
      <CustomModal width={DimensionHelper.wp(85)} isVisible={showEventEditModal} close={() => setShowEventEditModal(false)}>
        {recurrenceModalType && <EditRecurringModal
          ...
          onDone={(editType) => { (recurrenceModalType === "delete") ? handleRecurringDelete(editType) : handleRecurringSave(editType) }} />}
      </CustomModal>
    </View>
  )
}
