...
export const CheckinGroups = (props: Props) => {
      ...
      {selected == item.key &&
        item.items.map((item_group: any) => (
          <View key={`group-item-${item_group.id}`} style={{ backgroundColor: theme.colors.background }}>
            <Divider />
            <List.Item title={item_group.name} onPress={() => selectGroup(item_group)} style={{ backgroundColor: theme.colors.surfaceVariant }} />
          </View>
        ))}
    </View>
  );
  ...
  return (
    <View>
      ...
      <FlatList data={CheckinHelper.groupTree} renderItem={({ item }) => renderGroupItem(item)} keyExtractor={(item: any) => item.key} style={{ width: "100%" }} />
      <Button mode="contained" onPress={() => selectGroup(null)} style={{ marginTop: spacing.md }}>
        NONE
      </Button>
    </View>
  );
